...
from episia.data.types import (
    ...
    optimize_numeric_type,
    optimize_object_type,
)
...
from episia.viz.curves import plot_doubling, plot_epicurve, plot_incidence, plot_trend
...
from episia.viz.roc import plot_precision_recall, plot_roc, plot_roc_compare
...
def binary_data():
    ...
    y_score = np.concatenate(
        [np.random.beta(5, 2, 60), np.random.beta(2, 5, 60)])
...
@pytest.fixture
def seir_result():
    p = SEIRParameters(N=50_000, I0=1, E0=5, beta=0.35,
                       sigma=1/5.2, gamma=1/14, t_span=(0, 60))
    return SEIRModel(p).run()


@pytest.fixture
def seird_result():
    p = SEIRDParameters(N=50_000, I0=1, E0=5, beta=0.35,
                        sigma=1/5.2, gamma=0.09, mu=0.01, t_span=(0, 60))
...
class TestDiagnosticFromData:
    def test_returns_result(self, binary_data):
        ...
        assert isinstance(diagnostic_from_data(
            y_true, y_pred), DiagnosticResult)
    ...
    def test_perfect_classifier(self):
        ...
        assert r.sensitivity == pytest.approx(
            1.0) and r.specificity == pytest.approx(1.0)
...
class TestCompareDiagnosticTests:
    ...
    def test_same_test_zero_diff(self, d1):
        assert abs(compare_diagnostic_tests(d1, d1)[
                   'sensitivity_difference']) < 1e-10
...
class TestDirectStandardization:
    def test_adjusted_rate_positive(self):
        r = direct_standardization(
            np.array([0.02, 0.05, 0.10]), np.array(
                [1000, 500, 200]), np.array([2000, 1500, 800]),
        )
        assert hasattr(
            r, 'adjusted_rate') and r.adjusted_rate > 0 and hasattr(r, 'ci')


class TestIndirectStandardization:
    def test_smr_positive(self):
        r = indirect_standardization(
            np.array([25, 30, 20]), np.array(
                [1000, 500, 200]), np.array([0.02, 0.05, 0.08]),
        )
...
class TestStratifiedByVariable:
    def test_returns_stratified_table(self, stratified_df):
        ...
        st = stratified_by_variable(
            stratified_df, 'exposure', 'outcome', 'age_group')
...
class TestSensitivityDistributions:
    def _run(self, dist, n=6):
        sa = SensitivityAnalysis(SEIRModel, SEIRParameters,
                                 fixed=dict(N=50_000, I0=1, E0=5,
                                            t_span=(0, 60)),
                                 distributions={'beta': dist}, n_samples=n, seed=0)
        return sa.run(verbose=False)

    def test_uniform(self): assert self._run(
        ('uniform', 0.2, 0.5)).n_samples == 6

    def test_normal(self): assert self._run(
        ('normal', 0.35, 0.05)).n_samples == 6

    def test_triangular(self): assert self._run(
        ('triangular', 0.2, 0.35, 0.5)).n_samples == 6
...
class TestSensitivityResultMethods:
    ...
    def test_plot_all_compartments_plotly(self, sensitivity_result):
        ...
        for comp in sensitivity_result.compartment_names:
            assert isinstance(sensitivity_result.plot(
                comp, backend='plotly'), go.Figure)

    def test_plot_all_compartments_matplotlib(self, sensitivity_result):
        for comp in sensitivity_result.compartment_names:
            assert isinstance(sensitivity_result.plot(
                comp, backend='matplotlib'), matplotlib.figure.Figure)
    ...
    def test_metric_distribution_all_metrics_plotly(self, sensitivity_result):
        ...
        for metric in ['r0', 'peak_infected', 'peak_time', 'final_size']:
            assert isinstance(sensitivity_result.plot_metric_distribution(
                metric, backend='plotly'), go.Figure)

    def test_metric_distribution_all_metrics_matplotlib(self, sensitivity_result):
        for metric in ['r0', 'peak_infected', 'peak_time', 'final_size']:
            assert isinstance(sensitivity_result.plot_metric_distribution(
                metric, backend='matplotlib'), matplotlib.figure.Figure)
    ...
    def test_envelopes_is_dict(self, sensitivity_result):
        assert isinstance(sensitivity_result.envelopes, dict) and len(
            sensitivity_result.envelopes) > 0
...
class TestPlotlyPlotter:
    def test_plot_seir(self, seir_result):
        import plotly.graph_objects as go
        assert isinstance(get_plotter(
            'plotly').plot_model(seir_result), go.Figure)

    def test_plot_seird(self, seird_result):
        import plotly.graph_objects as go
        assert isinstance(get_plotter(
            'plotly').plot_model(seird_result), go.Figure)

    def test_plot_sir(self, sir_result):
        import plotly.graph_objects as go
        assert isinstance(get_plotter(
            'plotly').plot_model(sir_result), go.Figure)

    def test_plot_epicurve(self, ts_result):
        import plotly.graph_objects as go
        assert isinstance(get_plotter(
            'plotly').plot_epicurve(ts_result), go.Figure)

    def test_plot_roc(self, roc_result):
        import plotly.graph_objects as go
        assert isinstance(get_plotter(
            'plotly').plot_roc(roc_result), go.Figure)

    def test_plot_diagnostic(self, diag_result):
        import plotly.graph_objects as go
        assert isinstance(get_plotter(
            'plotly').plot_diagnostic(diag_result), go.Figure)

    def test_all_themes(self, seir_result):
        for theme in ['scientific', 'minimal', 'dark', 'colorblind']:
            fig = get_plotter('plotly').plot_model(
                seir_result, config=PlotConfig(theme=theme))
            assert fig is not None

    def test_roc_all_themes(self, roc_result):
        for theme in ['dark', 'minimal', 'colorblind']:
            assert get_plotter('plotly').plot_roc(
                roc_result, config=PlotConfig(theme=theme)) is not None

    def test_diagnostic_themes(self, diag_result):
        for theme in ['dark', 'minimal']:
            assert get_plotter('plotly').plot_diagnostic(
                diag_result, config=PlotConfig(theme=theme)) is not None
...
class TestMatplotlibPlotter:
    def test_plot_seir(self, seir_result):
        assert isinstance(get_plotter('matplotlib').plot_model(
            seir_result), matplotlib.figure.Figure)

    def test_plot_seird(self, seird_result):
        assert isinstance(get_plotter('matplotlib').plot_model(
            seird_result), matplotlib.figure.Figure)

    def test_plot_sir(self, sir_result):
        assert isinstance(get_plotter('matplotlib').plot_model(
            sir_result), matplotlib.figure.Figure)

    def test_plot_epicurve(self, ts_result):
        assert isinstance(get_plotter('matplotlib').plot_epicurve(
            ts_result), matplotlib.figure.Figure)

    def test_plot_roc(self, roc_result):
        assert isinstance(get_plotter('matplotlib').plot_roc(
            roc_result), matplotlib.figure.Figure)

    def test_plot_diagnostic(self, diag_result):
        assert isinstance(get_plotter('matplotlib').plot_diagnostic(
            diag_result), matplotlib.figure.Figure)

    def test_plot_contingency(self, table):
        assert isinstance(get_plotter('matplotlib').plot_contingency(
            table), matplotlib.figure.Figure)

    def test_all_themes(self, seir_result):
        for theme in ['scientific', 'minimal', 'dark', 'colorblind']:
            fig = get_plotter('matplotlib').plot_model(
                seir_result, config=PlotConfig(theme=theme))
    ...
    def test_roc_all_themes(self, roc_result):
        for theme in ['dark', 'minimal', 'colorblind']:
            assert isinstance(
                get_plotter('matplotlib').plot_roc(
                    roc_result, config=PlotConfig(theme=theme)),
...
class TestVizCurves:
    def test_epicurve_plotly(self, t_arr, v_arr):
        import plotly.graph_objects as go
        assert isinstance(plot_epicurve(
            times=t_arr, values=v_arr, backend='plotly'), go.Figure)

    def test_epicurve_matplotlib(self, t_arr, v_arr):
        assert isinstance(plot_epicurve(
            times=t_arr, values=v_arr, backend='matplotlib'), matplotlib.figure.Figure)
    ...
    def test_epicurve_themes(self, t_arr, v_arr):
        for theme in ['dark', 'minimal', 'colorblind']:
            assert plot_epicurve(times=t_arr, values=v_arr,
                                 theme=theme) is not None

    def test_trend_plotly(self, t_arr, v_arr):
        import plotly.graph_objects as go
        assert isinstance(plot_trend(
            times=t_arr, values=v_arr, backend='plotly'), go.Figure)

    def test_trend_matplotlib(self, t_arr, v_arr):
        assert isinstance(plot_trend(times=t_arr, values=v_arr,
                          backend='matplotlib'), matplotlib.figure.Figure)

    def test_trend_no_observed(self, t_arr, v_arr):
        assert plot_trend(times=t_arr, values=v_arr,
                          show_observed=False) is not None

    def test_incidence_plotly(self, t_arr, v_arr):
        import plotly.graph_objects as go
        assert isinstance(plot_incidence(
            times=t_arr, rates=v_arr/10000, backend='plotly'), go.Figure)

    def test_incidence_matplotlib(self, t_arr, v_arr):
        assert isinstance(plot_incidence(
            times=t_arr, rates=v_arr/10000, backend='matplotlib'), matplotlib.figure.Figure)

    def test_incidence_with_ci(self, t_arr, v_arr):
        rates = v_arr/10000
        assert plot_incidence(times=t_arr, rates=rates,
                              ci_lower=rates*0.8, ci_upper=rates*1.2) is not None

    def test_doubling_plotly(self):
        import plotly.graph_objects as go
        t = np.array([1., 2., 3., 4.])
        v = np.array([10., 20., 40., 80.])
        assert isinstance(plot_doubling(times=t, values=v,
                          doubling_time=1.0, backend='plotly'), go.Figure)

    def test_doubling_matplotlib(self):
        t = np.array([1., 2., 3., 4.])
        v = np.array([10., 20., 40., 80.])
        assert isinstance(plot_doubling(
            times=t, values=v, doubling_time=1.0, backend='matplotlib'), matplotlib.figure.Figure)
...
class TestVizRoc:
    ...
    def test_plot_roc_matplotlib(self, roc_result):
        assert isinstance(
            plot_roc(roc_result, backend='matplotlib'), matplotlib.figure.Figure)
    ...
    def test_compare_plotly(self, roc_result):
        import plotly.graph_objects as go
        assert isinstance(plot_roc_compare([roc_result, roc_result], labels=[
                          'A', 'B'], backend='plotly'), go.Figure)

    def test_compare_matplotlib(self, roc_result):
        assert isinstance(plot_roc_compare([roc_result, roc_result], labels=[
                          'A', 'B'], backend='matplotlib'), matplotlib.figure.Figure)
    ...
    def test_precision_recall_matplotlib(self, binary_data):
        y_true, y_score, _ = binary_data
        assert isinstance(plot_precision_recall(
            y_true, y_score, backend='matplotlib'), matplotlib.figure.Figure)
...
class TestVizForest:
    ...
    def test_meta_forest_custom_null(self):
        assert plot_meta_forest([0.5, 0.8], [0.3, 0.5], [0.8, 1.1], [
                                'A', 'B'], null_value=0.0) is not None
...
class TestVizContingencyPlot:
    ...
    def test_measures_matplotlib(self, table):
        assert isinstance(plot_measures(
            table, backend='matplotlib'), matplotlib.figure.Figure)
...
class TestVizUtils:
    def test_p_value_label(self):
        for p in [0.0001, 0.001, 0.03, 0.5]:
            assert isinstance(p_value_label(
                p), str) and 'p' in p_value_label(p).lower()
    ...
    def test_ci_band_xy(self):
        x, y = ci_band_xy(np.array([1., 2., 3.]), np.array(
            [0.5, 1., 1.5]), np.array([1.5, 3., 4.5]))
...
class TestVizThemes:
    ...
    def test_register_theme(self):
        register_theme('test_custom', palette=[
                       '#ff0000', '#00ff00', '#0000ff', '#ffff00'], bg_paper='#fff')
...
class TestDataTypes:
    ...
    def test_optimize_dataframe(self, sample_df):
        ...
        assert isinstance(r, pd.DataFrame) and set(
            r.columns) == set(sample_df.columns)
    ...
    def test_optimize_column_type_datetime(self):
        r = optimize_column_type(
            pd.Series(pd.date_range('2024-01-01', periods=4)))
        assert np.issubdtype(r.dtype, np.datetime64)

    def test_convert_to_epidemiological_types(self):
        df = pd.DataFrame({'cases': [1, 2, 3], 'region': [
                          'A', 'B', 'A'], 'rate': [0.1, 0.2, 0.3]})
    ...
    def test_convert_to_binary(self):
        assert set(convert_to_binary(
            pd.Series([True, False, True])).unique()).issubset({0, 1})

    def test_convert_to_categorical(self):
        assert convert_to_categorical(
            pd.Series(['A', 'B', 'A'])).dtype.name == 'category'

    def test_convert_to_continuous(self):
        assert np.issubdtype(convert_to_continuous(
            pd.Series([1, 2, 3])).dtype, np.number)

    def test_convert_to_date(self):
        assert np.issubdtype(convert_to_date(
            pd.Series(['2024-01-01', '2024-01-08'])).dtype, np.datetime64)
    ...
    def test_no_downcast_floats(self):
        ...
        assert isinstance(optimize_dataframe_types(
            df, downcast_floats=False), pd.DataFrame)
...
class TestDataTransformers:
    ...
    def test_normalize_minmax(self):
        ...
        assert r['a'].min() == pytest.approx(
            0.) and r['a'].max() == pytest.approx(1.)

    def test_outlier_clip(self):
        df = pd.DataFrame({'v': [1., 2., 100., 1.5]})
        r = OutlierTransformer(
            numeric_columns=['v'], action='clip').fit_transform(df.copy())
        assert r['v'].max() < 100.

    def test_outlier_remove(self):
        df = pd.DataFrame({'v': [1., 2., 100., 1.5, 1.8]})
        r = OutlierTransformer(
            numeric_columns=['v'], action='remove').fit_transform(df.copy())
        assert len(r) < len(df)

    def test_outlier_zscore_method(self):
        df = pd.DataFrame({'v': [1., 2., 3., 50., 2., 1., 3.]})
        r = OutlierTransformer(
            numeric_columns=['v'], method='zscore', threshold=2.0).fit_transform(df.copy())
        assert isinstance(r, pd.DataFrame)
    ...
    def test_pipeline_applies_transformers(self):
        df = pd.DataFrame(
            {'date': ['2024-01-01', '2024-01-08'], 'cases': [10, 20], 'region': ['A', 'B']})
        pipe = create_pipeline([DateTransformer(date_columns='date')])
