...
class TestCalculationStats:
    ...
    def test_hit_rate_zero_calls(
        self): assert CalculationStats().cache_hit_rate == 0.0

    def test_hit_rate_partial(self):
        assert CalculationStats(
            call_count=10, cache_hits=4).cache_hit_rate == pytest.approx(40.0)

    def test_hit_rate_100(self):
        assert CalculationStats(
            call_count=5, cache_hits=5).cache_hit_rate == pytest.approx(100.0)
    ...
    def test_avg_time_ms(self):
        ...
        assert CalculationStats(
            call_count=4, total_time=0.004).average_time == pytest.approx(1.0)
...
class TestBaseCalculator:
    ...
    def test_custom_strategy(self):
        assert BaseCalculator(
            CacheStrategy.NONE).cache_strategy == CacheStrategy.NONE
    ...
    def test_cached_method_none_strategy(self):
        c = BaseCalculator(CacheStrategy.NONE)
        calls = [0]
        def fn(self_, x): calls[0] += 1; return x
        w = c.cached_method(fn)
        w(c, 1)
...
class TestRiskRatio:
    def test_nominal(self, ecalc):
        assert ecalc.risk_ratio(
            40, 10, 20, 30) == pytest.approx(2.667, rel=1e-3)
...
class TestAttributableFraction:
    ...
    def test_paf_nominal(self, ecalc):
        ...
        assert ecalc.population_attributable_fraction(
            2.0, 0.5) == pytest.approx(1/3, rel=1e-3)

    def test_paf_rr_one(self, ecalc):
        assert ecalc.population_attributable_fraction(
            1.0, 0.5) == pytest.approx(0.0)

    def test_paf_zero_exposed(self, ecalc):
        assert ecalc.population_attributable_fraction(
            3.0, 0.0) == pytest.approx(0.0)
...
class TestProbabilities:
    def test_binomial_nominal(self, ecalc):
        assert ecalc.binomial_probability(
            5, 10, 0.5) == pytest.approx(0.2461, rel=1e-3)

    def test_binomial_sums_to_one(self, ecalc):
        assert sum(ecalc.binomial_probability(k, 5, 0.4)
                   for k in range(6)) == pytest.approx(1.0)
    ...
    def test_poisson_nominal(self, ecalc):
        assert ecalc.poisson_probability(
            3, 3.0) == pytest.approx(0.2240, rel=1e-3)

    def test_poisson_zero_k(self, ecalc):
        assert ecalc.poisson_probability(
            0, 2.0) == pytest.approx(math.exp(-2), rel=1e-6)
...
class TestSingletons:
    ...
    def test_matrix_works(self):
        assert matrix_calculator.effective_reproduction_number(
            3.0, 0.8) == pytest.approx(2.4)
...
class TestValidateProportion:
    ...
    def test_zero_allowed(self): assert validate_proportion(
        0.0) == pytest.approx(0.0)

    def test_one_allowed(self): assert validate_proportion(
        1.0) == pytest.approx(1.0)

    def test_above_one(self):
        with pytest.raises(ValidationError):
            validate_proportion(1.1)
    ...
    def test_zero_not_at_boundary(self):
        with pytest.raises(ValidationError):
            validate_proportion(0.0, allow_boundary=False)

    def test_one_not_at_boundary(self):
        with pytest.raises(ValidationError):
            validate_proportion(1.0, allow_boundary=False)

    def test_non_numeric(self):
        with pytest.raises(ValidationError):
            validate_proportion("high")

    def test_name_in_error(self):
        with pytest.raises(ValidationError, match="my_p"):
            validate_proportion(2.0, name="my_p")


# 9. validate_confidence_level

class TestValidateConfidenceLevel:
    def test_95(self): assert validate_confidence_level(
...
class TestValidateSampleSize:
    ...
    def test_returns_int(self): assert isinstance(
        validate_sample_size(100), int)
    ...
    def test_custom_min(self): assert validate_sample_size(
        30, min_size=30) == 30

    def test_below_min(self):
        with pytest.raises(ValidationError):
            validate_sample_size(29, min_size=30)

    def test_non_numeric(self):
        with pytest.raises(ValidationError):
            validate_sample_size("big")
...
class TestValidateBinaryVariable:
    ...
    def test_booleans(self):
        assert isinstance(validate_binary_variable(
            [True, False, True]), pd.Series)
    ...
    def test_numpy_array(self):
        assert isinstance(validate_binary_variable(
            np.array([0, 1, 0, 1])), pd.Series)

    def test_pandas_series(self):
        assert isinstance(validate_binary_variable(
            pd.Series([0, 1, 1, 0])), pd.Series)

    def test_not_array_like(self):
        with pytest.raises(ValidationError):
            validate_binary_variable(42)
...
class TestValidatePositive:
    ...
    def test_zero_non_strict(self): assert validate_positive(
        0.0, strict=False) == pytest.approx(0.0)
...
class TestCheckConvergence:
    ...
    def test_nan_false(self):
        assert check_convergence(
            np.array([1.0, float("nan")]), iteration=0) is False

    def test_inf_false(self):
        assert check_convergence(
            np.array([1.0, float("inf")]), iteration=0) is False

    def test_just_below_max(self):
        assert check_convergence(
            np.array([1.0]), max_iterations=10, iteration=9) is True
...
class TestExceptions:
    def test_base_raises(self):
        with pytest.raises(EpisiaError):
            raise EpisiaError("test")
