...
class TestSignificanceStars:
    ...
    def test_not_significant(self): assert significance_stars(
        0.5) in ('', 'ns', 'n.s.')
    def test_returns_string(self): assert isinstance(
        significance_stars(0.01), str)


class TestHexToRgb:
    def test_returns_tuple(self): assert isinstance(
        hex_to_rgb('#2997ff'), tuple)
    ...
    def test_known_value(self):      r, g, b = hex_to_rgb(
        '#2997ff'); assert r == 41 and g == 151 and b == 255


class TestHexToRgbaStr:
    def test_returns_string(self): assert isinstance(
        hex_to_rgba_str('#2997ff', 0.5), str)

    def test_starts_rgba(self): assert hex_to_rgba_str(
        '#2997ff', 0.5).startswith('rgba(')
    def test_alpha_included(
        self): assert '0.5' in hex_to_rgba_str('#2997ff', 0.5)


class TestAdjustAlpha:
    def test_returns_string(self): assert isinstance(
        adjust_alpha('#2997ff', 0.5), str)
    def test_rgba_format(self): assert adjust_alpha(
        '#2997ff', 0.5).startswith('rgba(')
...
class TestCiBandXy:
    def test_returns_tuple(self):
        r = ci_band_xy(np.array([1., 2., 3.]), np.array(
...
class TestNiceLogTicks:
    def test_returns_list(self): assert isinstance(
        nice_log_ticks(0.01, 100), list)
...
class TestSymlogRange:
    def test_returns_tuple(self): assert isinstance(
        symlog_range(np.array([-1., 0., 5.])), tuple)

    def test_two_values(self):      lo, hi = symlog_range(
        np.array([-1., 0., 5.])); assert lo < hi
...
class TestPlotRocCompare:
    def test_two_models_plotly(self, roc_result):
        import plotly.graph_objects as go
        fig = plot_roc_compare([roc_result, roc_result], labels=[
                               'A', 'B'], backend='plotly')
        assert isinstance(fig, go.Figure)

    def test_two_models_matplotlib(self, roc_result):
        fig = plot_roc_compare([roc_result, roc_result], labels=[
                               'A', 'B'], backend='matplotlib')
...
class TestPlotPrecisionRecall:
    ...
    def test_custom_label(self):
        ...
        assert plot_precision_recall(
            y_true, y_score, label='Classifier A') is not None
