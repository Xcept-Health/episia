...
if page == "Home":
    ...
    modules = [
        (
            ...
            {"en": "Disease burden map",
                "fr": "Carte de charge de morbidité"},
            {
        ...
        (
            ...
            {"en": "Weekly epidemic surveillance",
                "fr": "Surveillance épidémique hebdomadaire"},
            {
        ...
        (
            ...
            {"en": "Rapid diagnostic test performance",
                "fr": "Performance des tests diagnostiques rapides"},
            {
        ...
        (
            ...
            {"en": "SEIRD outbreak simulation",
                "fr": "Simulation de réponse épidémique SEIRD"},
            {
        ...
        (
            ...
            {"en": "HIV cascade · 90-90-90 targets",
                "fr": "Cascade de traitement VIH · cibles 90-90-90"},
            {
        ...
        (
            ...
            {"en": "Acute malnutrition screening (MUAC)",
             "fr": "Dépistage malnutrition aiguë (MUAC)"},
            {
        ...
        (
            ...
            {"en": "Sample size calculation",
                "fr": "Calcul de taille d'échantillon"},
            {
...
if page == "Disease Burden Globe":
    ...
    with col_ctrl:
        ...
        map_view = st.radio(
            "Map view", ["3D Globe", "Flat Africa map"], label_visibility="collapsed")
    ...
    with col_globe:
        ...
        fig_globe = go.Figure(go.Choropleth(
            ...
            colorbar=dict(
                title=dict(text=disease_unit[disease], font=dict(
                    size=9, color=PLT_FONT)),
...
# PAGE : Meningitis Outbreak
    ...
    from episia.models.parameters import ScenarioSet, SEIRParameters
    ...
    with col_p:
        ...
        district = st.selectbox("District", [
                                "Kaya (Centre-Nord)", "Dori (Sahel)", "Ouahigouya (Nord)", "Titao (Nord)"])
    ...
    with col_r:
        ...
        df_s = pd.DataFrame(
            {"date": dates, "cases": cases, "district": district})
        ds = SurveillanceDataset(
            df_s, date_col="date", cases_col="cases", district_col="district")
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.markdown(card(f"{cases.sum():,}", "Total cases",
                    f"Year {year}"), unsafe_allow_html=True)
        ...
        c3.markdown(card(f"{cases.max()}", "Peak cases/wk",
                    f"week {int(weeks[np.argmax(cases)])}"), unsafe_allow_html=True)
        c4.markdown(card(f"{attack_rate:.0f}", "Attack rate",
                    "per 100,000"), unsafe_allow_html=True)
        ...
        st.markdown(
            sec("Outbreak projection · intervention scenarios"), unsafe_allow_html=True)
        N_eff = int(population * (1 - vax_coverage/100))
        scenarios = ScenarioSet([
            ("No intervention",  SEIRParameters(N=population, I0=5, E0=20,
             beta=beta, sigma=sigma, gamma=gamma, t_span=(0, 120))),
            ("Mass vaccination", SEIRParameters(N=N_eff, I0=5, E0=20,
             beta=beta, sigma=sigma, gamma=gamma, t_span=(0, 120))),
            ("Ring vaccination", SEIRParameters(N=population, I0=5, E0=20,
             beta=beta*0.45, sigma=sigma, gamma=gamma, t_span=(0, 120))),
            ("Combined",         SEIRParameters(N=N_eff, I0=5, E0=20,
             beta=beta*0.45, sigma=sigma, gamma=gamma, t_span=(0, 120))),
        ])
        ...
        with col_t:
            ...
            df_display.columns = ["Scenario", "R₀",
                                  "Peak infected", "Peak day", "Final size"]
            df_display["R₀"] = df_display["R₀"].round(2)
            df_display["Peak infected"] = df_display["Peak infected"].apply(
                lambda x: f"{int(x):,}")
            df_display["Peak day"] = df_display["Peak day"].round(
                0).astype(int)
            df_display["Final size"] = (
                df_display["Final size"]*100).round(1).astype(str)+"%"
            st.dataframe(df_display.set_index(
                "Scenario"), use_container_width=True)
        with col_a:
            st.markdown('<div class="sec-hdr">Top alerts</div>',
                        unsafe_allow_html=True)
...
# PAGE : Vaccine Efficacy
    ...
    from episia.stats.contingency import Table2x2, odds_ratio, risk_ratio
    from episia.stats.stratified import mantel_haenszel_or

    tab_cohort, tab_stratified = st.tabs(
        ["Cohort Analysis", "Age-stratified · Mantel-Haenszel"])
    ...
    with tab_cohort:
        ...
        with col_p:
            ...
            st.markdown(
                f'<div style="font-size:.72rem;color:{T_SEC};margin-bottom:.4rem">Vaccinated group</div>', unsafe_allow_html=True)
            a = st.number_input("Cases (vaccinated)",
                                value=12,   min_value=0)
            b = st.number_input("Non-cases (vaccinated)",
                                value=2988, min_value=0)
            st.markdown(
                f'<div style="font-size:.72rem;color:{T_SEC};margin-bottom:.4rem">Unvaccinated group</div>', unsafe_allow_html=True)
            c = st.number_input("Cases (unvaccinated)",
                                value=87,   min_value=0)
            d = st.number_input("Non-cases (unvaccinated)",
                                value=2913, min_value=0)
            study_site = st.text_input(
                "Study site", value="Kaya District, Burkina Faso")
        ...
        with col_r:
            try:
                ...
                c1, c2, c3, c4 = st.columns(4)
                c1.markdown(card(f"{ve:.1f}%", "Vaccine Efficacy",
                            f"[{ve_lo:.1f}% – {ve_hi:.1f}%]"), unsafe_allow_html=True)
                c2.markdown(card(f"{rr.estimate:.3f}", "Risk Ratio",
                            f"p={fmt_p(rr.p_value)}"), unsafe_allow_html=True)
                c3.markdown(
                    card(f"{r_vacc:.1f}", "Risk vaccinated", "per 1,000"), unsafe_allow_html=True)
                c4.markdown(
                    card(f"{r_unv:.1f}", "Risk unvaccinated", "per 1,000"), unsafe_allow_html=True)

                if ve > 70 and rr.p_value < 0.05:
                    st.success(
                        f"High vaccine efficacy ({ve:.1f}%)  statistically significant (p={fmt_p(rr.p_value)}).")
                elif ve > 50:
                    st.warning(
                        f"Moderate vaccine efficacy ({ve:.1f}%). Consider coverage improvements.")
                elif ve < 0:
                    st.error(
                        "Negative VE  investigate cold chain or selection bias.")
                ...
                st.markdown(sec("VE estimates · regional comparison"),
                            unsafe_allow_html=True)
                studies = {
                ...
                for i, (sname, (est, lo, hi)) in enumerate(studies.items()):
                    clr = ACC if (study_site.split(
                        ",")[0] in sname or study_year in sname) else T_SEC
    ...
    with tab_stratified:
        st.markdown(
            f'<div style="font-size:.78rem;color:{T_SEC};margin-bottom:.8rem">Mantel-Haenszel pooled estimate controlling for age group.</div>', unsafe_allow_html=True)
        col_p, col_r = st.columns([1, 2])
        with col_p:
            st.markdown(sec("Data by age group"), unsafe_allow_html=True)
            st.markdown(
                f'<div style="font-size:.72rem;color:{T_SEC}">Children &lt; 5 years</div>', unsafe_allow_html=True)
            ...
            st.markdown(
                f'<div style="font-size:.72rem;color:{T_SEC}">Children 5–14 years</div>', unsafe_allow_html=True)
            ...
            st.markdown(
                f'<div style="font-size:.72rem;color:{T_SEC}">Adults 15+ years</div>', unsafe_allow_html=True)
        ...
        with col_r:
            ...
            from episia.stats.stratified import mantel_haenszel_or
            strata = [Table2x2(a1, b1, c1_v, d1), Table2x2(
                a2, b2, c2_v, d2), Table2x2(a3, b3, c3_v, d3)]
            ...
            c1x.markdown(card(f"{mh.common_or:.3f}", "Pooled OR (MH)",
                         f"[{lo_or:.3f} – {hi_or:.3f}]"), unsafe_allow_html=True)
            c2x.markdown(card(f"{pooled_ve:.1f}%", "Pooled VE",
                         f"[{pooled_ve_lo:.1f}% – {pooled_ve_hi:.1f}%]"), unsafe_allow_html=True)
            c3x.markdown(card(
                f"{mh.q_p_value:.3f}", "p heterogeneity", "Cochran Q"), unsafe_allow_html=True)

            ages_or = [Table2x2(a1, b1, c1_v, d1).odds_ratio(), Table2x2(
                a2, b2, c2_v, d2).odds_ratio(), Table2x2(a3, b3, c3_v, d3).odds_ratio()]
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(size=[10, 18, 10], symbol=[
                            "line-ew", "diamond", "line-ew"], color=ACC2),
...
# PAGE : Malaria RDT
    ...
    with col_p:
        ...
        st.markdown(
            f'<div style="font-size:.72rem;color:{T_SEC};margin-bottom:.3rem">vs. expert microscopy</div>', unsafe_allow_html=True)
        ...
        site = st.text_input(
            "Site",        value="CHR Koudougou, Burkina Faso")
        ...
        n_patients = st.number_input(
            "Daily patient volume", value=120, min_value=10)

    with col_r:
        try:
            d = diagnostic_test_2x2(tp=tp, fp=fp, fn=fn, tn=tn)
            c1, c2, c3, c4 = st.columns(4)
            c1.markdown(card(f"{d.sensitivity*100:.1f}%", "Sensitivity",
                        "True positive rate"), unsafe_allow_html=True)
            c2.markdown(card(f"{d.specificity*100:.1f}%", "Specificity",
                        "True negative rate"), unsafe_allow_html=True)
            c3.markdown(card(f"{d.lr_positive:.1f}",      "LR+",
                        "Likelihood ratio+"),  unsafe_allow_html=True)
            c4.markdown(card(f"{d.lr_negative:.3f}",      "LR-",
                        "Likelihood ratio-"),  unsafe_allow_html=True)

            prevs = np.linspace(prev_range[0]/100, prev_range[1]/100, 60)
            ppvs, npvs = [], []
            for p in prevs:
                ppv, npv = predictive_values_from_sens_spec(
                    d.sensitivity, d.specificity, p)
                ppvs.append(ppv*100)
                npvs.append(npv*100)

            fig = go.Figure()
            fig.add_trace(go.Scatter(x=prevs*100, y=ppvs,
                          name="PPV", line=dict(color=ACC, width=2.5)))
            fig.add_trace(go.Scatter(x=prevs*100, y=npvs,
                          name="NPV", line=dict(color=ACC2, width=2.5)))
            ...
            st.markdown(sec("Operational impact at your site"),
                        unsafe_allow_html=True)
            cols_op = st.columns(3)
            for i, prev_pct in enumerate([10, 20, 35]):
                prev_f = prev_pct / 100
                ppv_v, npv_v = predictive_values_from_sens_spec(
                    d.sensitivity, d.specificity, prev_f)
                expected_pos = int(n_patients * prev_f)
                false_neg = int(expected_pos * (1-d.sensitivity))
                false_pos = int((n_patients - expected_pos)
                                * (1-d.specificity))
                with cols_op[i]:
                    st.markdown(
            ...
            cats = ["Sensitivity", "Specificity",
                    "PPV @ 20%", "NPV @ 20%", "Accuracy"]
            ppv20, npv20 = predictive_values_from_sens_spec(
                d.sensitivity, d.specificity, 0.20)
            acc = (tp+tn)/(tp+fp+fn+tn)*100
            vals = [d.sensitivity*100, d.specificity *
                    100, ppv20*100, npv20*100, acc]
            ...
            fig2.update_layout(
                polar=dict(
                    radialaxis=dict(
                        range=[0, 100], gridcolor=PLT_GRID, tickfont=dict(color=PLT_FONT)),
                ...
                title=dict(
                    text=f"Diagnostic profile · {rdt_name}", font=dict(color=PLT_FONT)),
                height=320,
            )
            st.plotly_chart(fig2, use_container_width=True)
        except Exception as e:
...
# PAGE : Cholera
    ...
    from episia.models.parameters import ScenarioSet, SEIRDParameters
    from episia.models.scenarios import ScenarioRunner
    ...
    with col_p:
        ...
        location = st.selectbox("Location", [
                                "Niamey (Niger)", "N'Djamena (Chad)", "Tahoua (Niger)", "Maradi (Niger)"])
    ...
    with col_r:
        try:
            ...
            scenarios = ScenarioSet([
                ("No intervention", SEIRDParameters(N=N, I0=I0, E0=E0,
                 beta=beta, sigma=sigma, gamma=gamma, mu=mu, t_span=(0, t_end))),
                ("WASH only",       SEIRDParameters(N=N, I0=I0, E0=E0, beta=wash_b,
                 sigma=sigma, gamma=gamma, mu=mu*0.6, t_span=(0, t_end))),
                ("OCV only",        SEIRDParameters(N=ocv_N, I0=I0, E0=E0,
                 beta=beta, sigma=sigma, gamma=gamma, mu=mu, t_span=(0, t_end))),
                ("WASH + OCV",      SEIRDParameters(N=ocv_N, I0=I0, E0=E0,
                 beta=wash_b, sigma=sigma, gamma=gamma, mu=mu*0.6, t_span=(0, t_end))),
            ])
            ...
            pct_reduction = (1 - combined_d/baseline_d) * \
                100 if baseline_d > 0 else 0

            c1, c2, c3, c4 = st.columns(4)
            c1.markdown(card(
                f"{baseline_d:,}", "Deaths (no action)", "baseline"), unsafe_allow_html=True)
            c2.markdown(card(f"{combined_d:,}", "Deaths (combined)",
                        "WASH + OCV"), unsafe_allow_html=True)
            ...
            c4.markdown(card(f"{ocv_cov}%", "OCV coverage",
                        f"efficacy {ocv_eff}%"), unsafe_allow_html=True)
...
# PAGE : HIV Treatment Cascade
    ...
    with col_p:
        ...
        plhiv = st.number_input("PLHIV (estimated)",
                                value=plhiv_default, step=1_000)
    ...
    with col_r:
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.markdown(card(f"{plhiv:,}", "PLHIV",
                    f"estimated {year_c}"), unsafe_allow_html=True)
        c2.markdown(card(f"{d1}%·{d2}%·{d3}%", "Cascade",
                    "diagnosed·ART·suppressed"), unsafe_allow_html=True)
        c3.markdown(card(f"{overall:.1f}%", "Overall suppression",
                    "of all PLHIV"), unsafe_allow_html=True)
...
# PAGE : Child Malnutrition MUAC
    ...
    with col_p:
        ...
        site_m = st.text_input(
            "Health district", value="Titao district, Nord region")
        pop_under5 = st.number_input(
            "Population under 5", value=12_500, step=500)
        screening_cov = st.slider("Screening coverage (%)", 10, 100, 65)
        season = st.selectbox(
            "Season", ["Lean season (May–Sep)", "Harvest (Oct–Jan)", "Pre-lean (Feb–Apr)"])
        ...
        itfc_capacity = st.number_input(
            "ITFC beds (inpatient SAM)", value=25, min_value=5)
        otp_capacity = st.number_input(
            "OTP slots (outpatient SAM)", value=120, min_value=10)
        tsfp_capacity = st.number_input(
            "TSFP slots (MAM)", value=300, min_value=20)
    ...
    with col_r:
        ...
        season_adj = {
            "Lean season (May–Sep)": -4, "Harvest (Oct–Jan)": +3, "Pre-lean (Feb–Apr)": -1}
        ...
        status_colors = {"Emergency": ACC3,
                         "Serious": ACC4, "Acceptable": ACC2}
        sc = status_colors[gam_status]

        c1, c2, c3, c4 = st.columns(4)
        c1.markdown(card(f"{n_screened:,}", "Children screened",
                    f"{screening_cov}% coverage"), unsafe_allow_html=True)
        ...
        c3.markdown(card(f"{n_sam:,}", "SAM cases",
                    "MUAC < 115mm"), unsafe_allow_html=True)
        c4.markdown(card(f"{n_mam:,}", "MAM cases",
                    "115–125mm"), unsafe_allow_html=True)
        ...
        for edge in edges[:-1]:
            if edge < sam_cutoff:
                bar_colors.append(ACC3)
            elif edge < mam_cutoff:
                bar_colors.append(ACC4)
            else:
                bar_colors.append(ACC2)
        ...
        st.markdown(
            sec("Therapeutic feeding program caseload vs. capacity"), unsafe_allow_html=True)
        ...
        with col_cap1:
            ...
            ratio_colors = [ACC3 if r > 100 else ACC4 if r >
                            80 else ACC2 for r in ratios]
...
# PAGE : Sample Size Calculator
    ...
    with tab1:
        ...
        with col_p:
            ...
            pw = st.select_slider(
                "Power", [0.70, 0.80, 0.85, 0.90, 0.95], value=0.80)
        ...
        with col_r:
            try:
                res = sample_size_risk_ratio(
                    risk_unexposed=p0, risk_ratio=rr_exp, power=pw, alpha=al, r=ratio_c)
                n_cases = int(res.n_per_group) if res.n_per_group else int(
                    res.n_cases or 0)
                n_ctrl = int(n_cases * ratio_c)
                c1, c2, c3 = st.columns(3)
                c1.markdown(
                    card(f"{n_cases:,}", "Cases needed",   ""), unsafe_allow_html=True)
                c2.markdown(
                    card(f"{n_ctrl:,}",  "Controls needed", ""), unsafe_allow_html=True)
                c3.markdown(card(f"{n_cases+n_ctrl:,}",
                ...
                for rr_v in rrs:
                    try:
                        r_v = sample_size_risk_ratio(
                            risk_unexposed=p0, risk_ratio=rr_v, power=pw, alpha=al)
                ...
                st.plotly_chart(fig, use_container_width=True)
                st.info(
                    f"p₀={p0*100:.0f}%, RR={rr_exp}, α={al}, power={pw*100:.0f}% → **{n_cases:,} exposed** + **{n_ctrl:,} unexposed**.")
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    with tab2:
        ...
        with col_p:
            ...
            pw2 = st.select_slider(
                "Power", [0.70, 0.80, 0.85, 0.90, 0.95], value=0.80, key="pw2")
            al2 = st.select_slider(
                "Alpha", [0.01, 0.05, 0.10], value=0.05, key="al2")
        with col_r:
            try:
                res2 = sample_size_odds_ratio(
                    proportion_exposed_controls=p_ctrl, odds_ratio=or_exp, power=pw2, alpha=al2)
                n_c = int(res2.n_cases or 0)
                n_k = int(res2.n_controls or 0)
                c1, c2, c3 = st.columns(3)
                c1.markdown(card(f"{n_c:,}", "Cases",    ""),
                            unsafe_allow_html=True)
                c2.markdown(card(f"{n_k:,}", "Controls", ""),
                            unsafe_allow_html=True)
                c3.markdown(card(f"{n_c+n_k:,}", "Total",
                            ""), unsafe_allow_html=True)
                st.info(
                    f"{n_c:,} cases + {n_k:,} controls to detect OR={or_exp} (power={pw2*100:.0f}%, α={al2}).")
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    with tab3:
        col_p, col_r = st.columns([1, 2])
        with col_p:
            st.markdown(sec("Diagnostic test evaluation"),
                        unsafe_allow_html=True)
        ...
        with col_r:
            try:
                ...
                c1, c2, c3 = st.columns(3)
                c1.markdown(
                    card(f"{n_pos:,}", "Disease+ subjects", ""), unsafe_allow_html=True)
                c2.markdown(
                    card(f"{n_neg:,}", "Disease- subjects", ""), unsafe_allow_html=True)
                c3.markdown(card(f"{n_pos+n_neg:,}", "Total",
                            ""), unsafe_allow_html=True)
                st.info(
                    f"Sens={exp_sens*100:.0f}%, Spec={exp_spec*100:.0f}%, precision ±{prec*100:.0f}%, prevalence {prev_d*100:.0f}%.")
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    with tab4:
        col_p, col_r = st.columns([1, 2])
        with col_p:
            st.markdown(sec("Single proportion survey"),
                        unsafe_allow_html=True)
            exp_prop = st.slider("Expected proportion (%)", 1, 80, 30) / 100
            prec4 = st.slider("Precision ±(%)", 2, 15, 5) / 100
            al4 = st.select_slider(
                "Alpha", [0.01, 0.05, 0.10], value=0.05, key="al4")
            deff = st.slider(
                "Design effect (cluster sampling)", 1.0, 3.0, 1.0, 0.1)
        with col_r:
            try:
                ...
                c1, c2 = st.columns(2)
                c1.markdown(card(f"{n_req:,}", "Required n",
                            f"DEFF ×{deff}"), unsafe_allow_html=True)
                c2.markdown(card(f"±{prec4*100:.0f}%", "Precision",
                            "95% CI half-width"), unsafe_allow_html=True)

                props = np.linspace(0.05, 0.70, 40)
                ns4 = []
                for pp in props:
                    try:
                        r = sample_size_single_proportion(expected_proportion=float(
                            pp), precision=prec4, alpha=al4, design_effect=deff)
                ...
                fig.add_vline(x=exp_prop*100, line_dash="dash",
                              line_color=ACC3)
