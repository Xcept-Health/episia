...
Voici le **formulaire complet** avec toutes les corrections intégrées en un seul bloc prêt à copier-coller.

Tu peux tout sélectionner et le coller directement dans le nouvel Issue sur le repo pyOpenSci (https://github.com/pyOpenSci/software-submission).

---

**Submitting Author:** (@ArielShadrac)  
**All current maintainers:** (@ArielShadrac)  
**Package Name:** Episia  
...
## Description
Episia is an open-source Python library for epidemiologists, public health researchers, and biostatisticians. It offers a modern, unified, and fully offline toolkit covering the full epidemiological workflow: processing raw surveillance data (with DHIS2 support), classical biostatistical calculations (risk/odds ratios, confidence intervals, diagnostic accuracy, sample size), compartmental epidemic modeling (SIR/SEIR/SEIRD), Monte Carlo sensitivity analysis, alert detection, and generation of self-contained HTML reports. Core statistical functions are systematically validated against the reference OpenEpi tool. Episia is specifically designed for practical use in resource-limited settings, including public health systems in Africa.
...
**Domain Specific**  
(Aucune case cochée – Geospatial et Education ne correspondent pas)
...
Episia falls under **Data processing/munging**, **Data visualization**, and **Scientific software wrappers** because it transforms raw surveillance data into structured epidemiological datasets, provides rich domain-specific visualizations (epidemic curves, ROC curves, endemic channels) with dual Plotly/Matplotlib backends, and offers a clean Python interface that wraps and significantly extends the validated calculations from the widely-used OpenEpi tool.

- **Who is the target audience and what are scientific applications of this package?**  
  The target audience includes field epidemiologists, public health officers, biostatisticians, students, and researchers, particularly those working in low-resource or African health systems. Scientific applications include outbreak investigation, routine disease surveillance and alert systems, epidemic modeling and forecasting, diagnostic test evaluation, sample size planning, and automated production of epidemiological bulletins for ministries of health.

- **Are there other Python packages that accomplish the same thing? If so, how does yours differ?**  
  The closest tool is OpenEpi (a popular web-based JavaScript application). Episia reimplements and validates its core epidemiological measures in Python while adding substantial new functionality: compartmental models (SIR/SEIR/SEIRD), Monte Carlo sensitivity analysis, DHIS2 integration for real surveillance data, unified result objects, alert engines, and automated reporting. General statistical packages (statsmodels, scipy) lack these domain-specific, validated epidemiological tools and the integrated public-health workflow. No other comprehensive, offline-first Python library combines these features at this level.
...
- [x] has a test suite.  
- [x] has continuous integration setup (GitHub Actions).

## Publication Options
- [ ] Do you wish to automatically submit to the [Journal of Open Source Software][JournalOfOpenSourceSoftware]?  
  (Laisse décoché pour le moment, ou coche si tu veux ajouter un `paper.md` plus tard)
...
- [ ] [Last but not least please fill out our pre-review survey](https://forms.gle/F9mou7S3jhe8DMJ16).

---

**Conseils avant soumission :**
- Mets à jour la version dans ton README (0.1.1).
- Remplis le survey Google Form.
- Crée un **nouvel Issue** sur https://github.com/pyOpenSci/software-submission en utilisant le template “Full submission” et colle tout le texte ci-dessus.

Si tu veux que j’ajoute le `paper.md` pour JOSS ou que je modifie quelque chose (par exemple cocher JOSS), dis-le-moi.

Tu es prêt à soumettre ? Colle ce texte et envoie ! 🚀  
Bon courage pour la review !