...
class StratifiedTable:
    ...
    def __post_init__(self):
        ...
        if self.strata_names is None:
            self.strata_names = [
                f"Stratum_{i+1}" for i in range(len(self.tables))]
        elif len(self.strata_names) != len(self.tables):
            raise ValueError(
                "Number of stratum names must match number of tables")
...
def mantel_haenszel_or(
    ...
    if len(stratum_or) > 1:
        ...
        if cochran_q > df:
            c = sum(stratum_weights) - \
                sum(w**2 for w in stratum_weights) / sum(stratum_weights)
...
def _cochran_q_test(stratum_or: List[float], weights: List[float]) -> float:
    ...
    weighted_mean = sum(w * np.log(or_val)
                        for w, or_val in zip(weights, stratum_or)) / sum(weights)

    Q = sum(w * (np.log(or_val) - weighted_mean) **
            2 for w, or_val in zip(weights, stratum_or))
...
def _woolf_test(stratified_tables: StratifiedTable) -> Dict[str, float]:
    ...
    weighted_mean = sum(
        w * val for w, val in zip(stratum_weights, log_or)) / sum(stratum_weights)

    # Woolf's chi-square
    chi2 = sum(w * (val - weighted_mean)**2 for w,
               val in zip(stratum_weights, log_or))
    df = len(stratum_or) - 1
...
def direct_standardization(
    ...
    # Crude rate
    crude_rate = np.sum(stratum_rates * stratum_populations) / \
        np.sum(stratum_populations)

    # Directly adjusted rate
    adjusted_rate = np.sum(
        stratum_rates * standard_population) / np.sum(standard_population)
...
def indirect_standardization(
    ...
    if total_observed >= 10:
        z = stats.norm.ppf(1 - (1 - confidence) / 2)
        ci_lower = total_observed * \
            (1 - 1/(9*total_observed) - z /
             (3*np.sqrt(total_observed)))**3 / expected_cases
        ci_upper = (total_observed + 1) * (1 - 1/(9*(total_observed+1)) +
                                           z/(3*np.sqrt(total_observed+1)))**3 / expected_cases
    else:
        # Exact Poisson CI
        ci_lower = stats.chi2.ppf(
            (1-confidence)/2, 2*total_observed) / (2*expected_cases)
        ci_upper = stats.chi2.ppf(
            1-(1-confidence)/2, 2*(total_observed+1)) / (2*expected_cases)
...
def stratified_by_variable(
    ...
    for stratum, group in data.groupby(stratify_var):
        ...
        table = Table2x2(
            a=group[(group[exposure_var] == 1) & (
                group[outcome_var] == 1)].shape[0],
            b=group[(group[exposure_var] == 0) & (
                group[outcome_var] == 1)].shape[0],
            c=group[(group[exposure_var] == 1) & (
                group[outcome_var] == 0)].shape[0],
            d=group[(group[exposure_var] == 0) & (
                group[outcome_var] == 0)].shape[0]
        )
