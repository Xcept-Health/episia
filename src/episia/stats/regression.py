...
def logistic_regression(
    ...
    if add_intercept:
        ...
        if variable_names is not None:
            ...
            variable_names = ["Intercept"] + \
                [f"X{i}" for i in range(X.shape[1] - 1)]
    ...
    if method == "irls":
        beta, converged, iterations, log_likelihood = _irls_logistic(
            X, y, beta, max_iter, tol)
    elif method == "newton":
        beta, converged, iterations, log_likelihood = _newton_logistic(
            X, y, beta, max_iter, tol)
    else:
    ...
    for g in range(n_groups):
        ...
        if expected > 0:
            chi2 += (observed - expected)**2 / \
                (expected * (1 - expected / len(group_true)))
